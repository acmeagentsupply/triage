...
octu_status_text_sessions_total() {
  local status_path="${1}"
...
patterns = [
    r"Agents\s+.*?\bsessions\s+(\d+)\b",
    r"Sessions\s+.*?\b(\d+)\s+active\b",
]
...
octu_contradiction_summary() {
  local bundle_dir="$1"
  local status_file="$2"
  local sessions_total status_total

  sessions_total="$(octu_status_value "$status_file" sessions total)"
  status_total="$(octu_status_text_sessions_total "${bundle_dir}/openclaw_status.txt" 2>/dev/null || true)"
  sessions_total="$(octu_status_numeric_or_zero "${sessions_total}")"
  status_total="$(octu_status_numeric_or_zero "${status_total}")"

  if (( sessions_total == 1 && status_total > 1 )); then
    printf 'CONTRADICTION_DETECTED (sessions=%s openclaw_status=%s)\n' "${sessions_total}" "${status_total}"
  fi
}

octu_bundle_confidence() {
  local bundle_dir="$1"
  local status_file="$2"
  local confidence

  if [[ -n "$(octu_contradiction_summary "${bundle_dir}" "${status_file}")" ]]; then
    printf 'LOW'
    return 0
  fi
  ...
  confidence="$(awk '
    ...
    END {
      if (low) print "LOW"
      else if (seen_medium) print "MEDIUM"
    }
...
# ── Share Mode Snapshot (triage -share) ────────────────────────────────────
...
" 2>/dev/null || echo "${disk%% *}")
  ...
  printf 'bundle: %s\n'        "${bundle_path}"
  local _ver
  _ver="$(cat "${HOME}/.openclaw/workspace/triage/VERSION" 2>/dev/null \
    || cat "$(dirname "$(dirname "$(realpath "${BASH_SOURCE[0]}" 2>/dev/null || echo /dev/null)")")/VERSION" 2>/dev/null \
    || echo "0.1.6")"
  printf 'generated by triage v%s | acmeagentsupply.com\n' "${_ver}"
}
...
octu_render_from_statuses() {
  ...
  [[ -n "$sessions_confidence" ]] || sessions_confidence="UNKNOWN"
  contradiction="$(octu_contradiction_summary "$bundle_dir" "$status_file")"
  bundle_confidence="$(octu_bundle_confidence "$bundle_dir" "$status_file")"
  ...
  if [[ "$gateway_state" == FAIL* || "$gateway_state" == DOWN* ]]; then
    ...
    status="DEGRADED"
    reason="gateway=WARN"
  elif [[ "$sessions_state" == INCOMPLETE* ]]; then
    status="DEGRADED"
